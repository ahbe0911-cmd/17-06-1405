    ...
    private void moveToForeground() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            try {
                ...
                Log.d("ZaffranGram", "Failed to move push service to foreground");
            }
        }
        foregroundStartPending = false;
    ...
    private Notification getNotification() {
        if (notification == null) {
            ...
            NotificationCompat.Builder builder = new NotificationCompat.Builder(this, CHANNEL_ID)
                    ...
                    .setContentTitle("Zaffran Gram")
                    .setContentText("Push service is running");
            try {
                Intent explainIntent = new Intent(this, LaunchActivity.class);
                explainIntent.setAction(Intent.ACTION_MAIN);
                builder.setContentIntent(PendingIntent.getActivity(this, 0, explainIntent, pendingIntentFlags));
            } catch (Throwable ignore) {
                Log.d("ZaffranGram", "Failed to set intent");
            }
